...
const ImageCard = (props) => {
  ...
  return (
    <div class="max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
      <img class="object-cover object-center w-full h-56" src={picture.url} alt={picture.title + "not available"} />

      <div class="flex items-center px-6 py-3 bg-gray-900">
        <h1 class="text-lg font-semibold text-white">{picture.title}</h1>
      </div>

      <div class="px-6 py-4">
        <p class="text-gray-700 dark:text-gray-400 text-sm line-clamp-4">{!picture.explanation ? "No description available." : picture.explanation}</p>
        ...
        <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
          ...
          <h1 class="px-2 text-sm">{!picture.copyright ? "No copyright." : `${picture.copyright}`}</h1>
        </div>
        ...
        <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
          ...
          <h1 class="px-2 text-sm">{!picture.date ? "" : new Date(picture.date).toDateString()}</h1>
        </div>
